...
pub const SymmetricState2 = struct {
    allocator: Allocator,
    cipher_choice: [10]u8,
    ...
    pub fn init(allocator: Allocator, protocol_name: []const u8) !Self {
        ...
        var h = try BoundedArray(u8, 64).init(hash_len);
        var ck = try BoundedArray(u8, 64).init(hash_len);
        if (protocol_name.len <= hash_len) {
            ...
            @memcpy(h.slice(), data[0..hash_len]);
        } else {
    ...
    pub fn mixKey(
        ...
        const output = try self.hasher.HKDF(self.allocator, self.ck.slice(), input_key_material, 2);

        self.ck = output[0];
        var temp_k: [32]u8 = undefined;
        if (self.hasher.len == 64) @memcpy(&temp_k, output[1].slice()[0..32]) else @memcpy(&temp_k, output[1].slice());
        self.cipher_state = CipherState.init(&self.cipher_choice, self.allocator, temp_k);
    }

    pub fn mixHash(self: *Self, data: []const u8) !void {
        const h_with_data = try std.mem.concat(self.allocator, u8, &[_][]const u8{ self.h.slice()[0..], data });
    ...
    pub fn mixKeyAndHash(self: *Self, input_key_material: []const u8) void {
        const output = self.hasher.HKDF(self.allocator, self.ck, input_key_material, 2);
    ...
    pub fn encryptAndHash(self: *Self, plaintext: []const u8) ![]const u8 {
        ...
        const ciphertext = try self.cipher_state.encryptWithAd(self.h.slice(), plaintext);
    ...
    pub fn split(
        ...
        const output = try self.hasher.HKDF(self.allocator, self.ck.slice()[0..], &[_]u8{}, 2);
...
test "init symmetric state" {
    var symmetric_state = try SymmetricState2.init(
